...
OutputLabel = ps.Text(key="OL")

layout = [[lable1],[InputBox1,Browse],[lable2],[InputBox2,DestFolder],[Compress],[OutputLabel]]
...
while True:
    ...
    for element in window.AllKeysDict:
        ps.Print(f"Element Key: {element}")
    if event =='Compress':
        filepaths = values["Browse"].split(';')
        print(filepaths)
        folder=values['DF']
        zipfile_function.make_zip(filepaths,folder)
        window["OL"].update(value="Compression Completed")
